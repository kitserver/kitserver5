...
#define DEFAULT_EXE_NAME "pes5.exe\0"
#define BUFLEN 4096
...
KitServer will NOT be attached to it.", fileName);
		...
		MessageBox(hWnd, buf, "KitServer 5 Setup Message", 0);
		return;
	}
	...
	if (f != NULL)
	{
		...
		if (dataOffset != 0) {
			...
			else
			{
				...
				sprintf(buf, "\
======== INFORMATION! =========\n\
KitServer 5 is already installed (1) for\n\
%s.", fileName);
		...
		if (codeOffset != 0) {
			...
			else
			{
				...
				sprintf(buf, "\
======== INFORMATION! =========\n\
KitServer 5 is already installed (2) for\n\
%s.", fileName);
        ...
        */
		...
		sprintf(buf, "\
======== SUCCESS! =========\n\
Setup has installed KitServer 5 for\n\
%s.", fileName);
	...
	else
	{
		...
		sprintf(buf, "\
======== ERROR! =========\n\
Setup failed to install KitServer 5 for\n\
%s.\n\
\n\
	...
	if (f != NULL)
	{
		...
		if (dataOffset != 0) {
			...
			if (newEntryPoint == 0)
			{
				...
				sprintf(buf, "\
======== INFORMATION! =========\n\
KitServer 5 is not installed for\n\
%s.", fileName);
        ...
        */
		...
		sprintf(buf, "\
======== SUCCESS! =========\n\
Setup has removed KitServer 5 from\n\
%s.", fileName);
	...
	else
	{
		...
		sprintf(buf, "\
======== ERROR! =========\n\
Setup failed to remove KitServer 5 from\n\
%s.\n\
\n\
(No modifications made.)\n\
Verify that the executable is not\n\
...
void InitControls(void)
{
	...
	while(true)
	{
        ...
        if (lstrcmpi(fData.cFileName, patchExeName) == 0) { // auto-select pes5.exe
